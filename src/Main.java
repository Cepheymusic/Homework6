
public class Main {
    public static void main(String[] args) {
        task1();
        task2();
        task3();
        task4();
        task5();
        task6();
        task7();
        task8();
        task9();
        task10();
    }
    public static void task1 () {
        System.out.println("Задача1");
        for (int i = 1; i <= 10; i++) {
            System.out.println(i);
        }
    }
    public static void task2 () {
        System.out.println("Задача2");
        for (int i = 10; i >= 1; i--) {
            System.out.println(i);
        }
    }
    public static void task3 () {
        System.out.println("Задача3");
        for (int i = 0; i <= 17; i++) {
            System.out.println(i);
        }
    }
    public static void task4 () {
        System.out.println("Задача4");
        for (int i = 10; i >= -10; i--) {
            System.out.println(i);
        }
    }
    public static void task5 () {
        System.out.println("Задача5");
        for (int i = 1994; i <= 2096; i = i + 4) {
            System.out.println(i + " год является високосным");
        }
    }
    public static void task6 () {
        System.out.println("Задача6");
        for (int i = 7; i <= 98; i = i + 7) {
            System.out.println(i + " год является високосным");
        }
    }
    public static void task7 () {
        System.out.println("Задача7");
        for (int i = 1; i <= 512; i = i * 2) {
            System.out.println(i + " год является високосным");
        }
    }
    public static void task8 () {
        System.out.println("Задача8");
        int monthlyAccumulation = 29000;
        int totalAccumulation = 0;
        for (int i = 1; i <= 12; i++) {
            totalAccumulation = totalAccumulation + monthlyAccumulation;
            System.out.println("Месяц " + i + " ,сумма накоплений равна " + totalAccumulation);
        }
    }
    public static void task9 () {
        System.out.println("Задача9");
        int monthlyAccumulation = 29000;
        int totalAccumulation = 0;
        for (int i = 1; i <= 12; i++) {
            monthlyAccumulation = monthlyAccumulation + monthlyAccumulation/100;
            totalAccumulation = totalAccumulation + monthlyAccumulation;
            System.out.println("Месяц " + i + " ,сумма накоплений равна " + totalAccumulation);
        }
    }
    public static void task10 () {
        System.out.println("Задача10");
        int number = 2;
        int result;
        for (int i = 1; i <= 10; i++) {
            result = number * i;
            System.out.println("2*" + i + "=" + result);
        }
    }


}